#'
#' @title \verb{markcorr} for data 7 test 6
#'
#' @description
#' Estimates Schlather I mark correlation function for univariate pattern
#' with two quantitative marks.
#'
#' @usage
#' mkcorr.d7.t6(X, m.qti.1="DBH", m.qti.2="Disease", r=NULL,
#'     correction=c("isotropic", "Ripley", "translate"),
#'     method="density", ...,
#'     weights=NULL, normalise=TRUE, internal=NULL)
#'
#' @param X An object of class "\verb{ppp}"，with two quantitative marks.
#' @param m.qti.1 A mark of data "\verb{X}" indicating the first quantitative mark.
#' @param m.qti.2 A mark of data "\verb{X}" indicating the second quantitative mark.
#' @param r Same as in \verb{markcorr()} from the package \verb{spatstat}.
#' @param correction Same as \verb{markcorr()} from the package \verb{spatstat}.
#' @param method Same as \verb{markcorr()} from the package \verb{spatstat}.
#' @param ... Same as \verb{markcorr()} from the package \verb{spatstat}.
#' @param weights Same as \verb{markcorr()} from the package \verb{spatstat}.
#' @param normalise Same as \verb{markcorr()} from the package \verb{spatstat}.
#' @param internal Same as \verb{markcorr()} from the package \verb{spatstat}.
#'
#' @details
#' The data type and test function are referred to (Wiegand and Moloney, 2013).
#' Data 7 is an univariate pattern with two quantitative marks. Test 6 is Schlather I:
#' \deqn{t(m_{il},m_{jm})=(m_{il}-\mu_l(r))(m_{jm}-\mu_m(r))}
#'
#' This estimation is time-cost.
#'
...
#'
#' @seealso \code{\link{mkcorr.d7}}, \code{\link{mkcorr.d8.t6}},
...
#' @examples
#' \dontrun{
#' plot(mkcorr.d7.t6(ppp.ash.g1,m.qti.1="dbh",m.qti.2="count_insect"))
#' nsim=2499
#' ev.mkc.t6.insectnum.dbh.g1<-envelope(ppp.ash.g1,mkcorr.d7.t6,
#'     funargs=list(m.qti.1="dbh",m.qti.2="count_insect"), nsim=nsim,
#'     simulate=rlabel)
#' plot(ev.mkc.t6.insectnum.dbh.g1)
#' }
#'
mkcorr.d7.t6<-function(X, m.qti.1="DBH", m.qti.2="Disease",r=NULL,
                   correction=c("isotropic", "Ripley", "translate"),
                   method="density", ...,
                   weights=NULL, normalise=TRUE, internal=NULL) {
