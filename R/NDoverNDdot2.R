#'
#' @title \verb{NDdot} ration estimation, untidy data
#'
#' @description
#' Estimates ratio between two dot-type \emph{k}th neareast neighbor distance functions (\verb{NDdot}).
#'
#' @usage
#' NDoverNDdot2(ppp.marks,V.species="Species", host="Ash", V.bistate="Infected",
#'     yes="Yes",no="No",...,
#'     r = NULL, breaks = NULL,
...
#'
#' @details
#' Bivariate difference \emph{ND} function is estimated as (Raventós, et al., 2010).
...
#'
#' @seealso \code{\link{NDoverNDdot}} which does the same job,
#' \code{\link{NDdot}}, \code{\link{govergdot2}}, \code{\link{LoverLdot2}}, \code{\link{JoverJdot2}},
...
#' @examples
#' \dontrun{
#' plot(NDoverNDdot2(ppp.marks.c3))
#' nsim=19
#' ev.nd_nddot2.ash.c3<-envelope(ppp.marks.c3,NDoverNDdot2,nsim=nsim,
...
#' #plot(ev.ndovernddot2.ash.c3)
#' }
#'
NDoverNDdot2<-function(ppp.marks,V.species="Species", host="Ash",
  ...
  if(is.null(yes)) yes<-levels(marks(ppp.marks)[1,V.bistate])[2]
  if(is.null(no)) no<-levels(marks(ppp.marks)[1,V.bistate])[1]
