#' @title Cumulative Density Correlation Function, data 8
#'
#' @description
#' Estimates cum. density correlation function for univariate pattern with one quantitative marks.
#'
#' @usage
#' mkcorr.d8.t7(X, m.qli="Infected", m.qti="DBH", ind=TRUE,
#'     r=NULL, verbose = TRUE, correction="Ripley",
#'     method="density",
...
#' @param dens.global Logical, indicating whether desnity is global or local.  If TRUE, \eqn{K(r)=K_{l+m}(r)} ; else \eqn{K(r)=K_l(r)} or\eqn{K(r)=K_m(r)}
#'
#' @details
...
#' \deqn{C_{m,K}(r)=(m_{il}-\mu_l)(K_{jm}(r)-K_m(r))}
#' \deqn{C_{m,K}(r)=(m_{il}-\mu_l)(K_{jm}(r)-K_{l+m}(r))}
#' \deqn{C_{m,K}(r)=(m_{il}-\mu_{l+m})(K_{jm}(r)-K_m(r))}
#' \deqn{C_{m,K}(r)=(m_{il}-\mu_{l+m})(K_{jm}(r)-K_{l+m}(r))}
#' where the function \eqn{K_i()} is the local Ripley \emph{K} function of
#' point \emph{i} and \eqn{K()} is the Ripley \emph{K} function which equals
#' to the mean of \eqn{K_i()}, \eqn{m_{il}} is the mark value of point \emph{i}
...
#'
#' @seealso \code{\link{mkcorr.d8.t8}}, \code{\link{mkcorr.d7.t7}},
#' \code{\link{mkcorr.d8.t6}}, \code{\link{mkcorr.t7}}.
#'
...
#' \dontrun{
#' nsim=2499
#' ev.mkc.d8t7.c3<-envelope(ppp.ash.infect.dbh.c3, m.qli="Infected", m.qti="DBH",
#'     mkcorr.d8.t7,
...
#' }
#'
mkcorr.d8.t7<-function(X, m.qli="Infected", m.qti="DBH", ind=TRUE,
                   r=NULL, verbose = TRUE,
                   correction="Ripley",
  ...
  if(dens.global){
    ...
    dIJ.lev<-levels(dIJ.fac)
    dIJ.uni<-as.numeric(dIJ.lev)
    m.J.localK<-matrix(NA,nrow=npts.no,ncol=length(dIJ.uni))
    for (i in 1: length(dIJ.uni)){
      m.J.localK[,i]<-localK(X.no,rmax=rmax, correction=correction,verbose = verbose,
                             rvalue=dIJ.uni[i])
    }
    m.J.localK<-m.J.localK[,dIJ.pos]
    m.J.Kest<-apply(m.J.localK,2,mean)
